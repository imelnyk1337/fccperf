...
processList = {
    #'p8_ee_ZZ_ecm240':{'fraction': 1.0},#Run the full statistics in one output file named <outputDir>/p8_ee_ZZ_ecm240.root
    #'p8_ee_WW_ecm240':{'fraction': 0.1, 'chunks':2}, #Run 50% of the statistics in two files named <outputDir>/p8_ee_WW_ecm240/chunk<N>.root
    'p8_ee_ZH_ecm240':{'fraction': 1.0, 'output':'p8_ee_ZH_ecm240_out'} #Run 20% of the statistics in one file named <outputDir>/p8_ee_ZH_ecm240_out.root (example on how to change the output name)
}
...
class RDFanalysis():
    ...
    def analysers(df):  
        df2 = (df
            # SaveGraph() as possible close to the begging
